...
const isStorageExist = () => {
  if (typeof Storage === undefined) {
    Swal.fire("Error", "Browser kamu tidak mendukung web storage", "error");
    return false;
  }
  return true;
};
...
const makeBookElement = (bookObject) => {
  ...
  elementBookTitle.innerHTML = `Judul Buku : ${bookObject.title} <span>(${bookObject.year})</span>`;

  const elementBookAuthor = document.createElement("h5");
  elementBookAuthor.classList.add("item-writer");
  elementBookAuthor.innerText = `Penulis : ${bookObject.author}`;
  ...
  if (bookObject.isComplete) {
    ...
    returnBtn.innerHTML = `Kembalikan Buku`;
    ...
    finishBtn.innerHTML = `Selesai Dibaca`;
